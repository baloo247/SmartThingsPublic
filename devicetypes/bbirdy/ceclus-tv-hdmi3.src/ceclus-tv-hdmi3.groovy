...
metadata {
  definition (name: "Ceclus TV HDMI3", namespace: "BBirdy", author: "Bal Birdy") {
    capability "Switch"
...
tiles(scale:2) {
 ...
 def on() {
    ...
    def hdmi3 = new physicalgraph.device.HubAction(
            method: "POST",
            path: "/apps/vr/browserseturl",
            body: ("""<?xml version="1.0" ?><remote><key code="1005"/></remote>"""),
            headers: ["Content-Type":"text/plain"+" ; "+"charset=ISO-8859-1",Host:"${ipadd}:${smartapp_port}"]
    )
    log.debug hdmi3.toString()
    sendHubCommand(hdmi3);
}