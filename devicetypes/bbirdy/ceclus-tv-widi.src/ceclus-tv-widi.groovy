...
metadata {
  definition (name: "Ceclus TV WIDI", namespace: "BBirdy", author: "Bal Birdy") {
    capability "Switch"
...
tiles(scale:2) {
 ...
 def on() {
    ...
    def widi = new physicalgraph.device.HubAction(
            method: "POST",
            path: "/apps/vr/browserseturl",
            body: ("""<?xml version="1.0" ?><remote><key code="1009"/></remote>"""),
            headers: ["Content-Type":"text/plain"+" ; "+"charset=ISO-8859-1",Host:"${ipadd}:${smartapp_port}"]
    )
    log.debug source.toString()
    sendHubCommand(widi);
